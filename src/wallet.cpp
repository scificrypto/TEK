...
#include "base58.h"
#include "coincontrol.h" //presstab
#include "kernel.h"
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl) const //presstab
{
    vCoins.clear();

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
                if (!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) && pcoin->vout[i].nValue > 0 && //presstab
				(!coinControl || !coinControl->HasSelected() || coinControl->IsSelected((*it).first, i))) //presstab
...
bool CWallet::SelectCoins(int64 nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet, const CCoinControl* coinControl) const //presstab
{
    vector<COutput> vCoins;
	AvailableCoins(vCoins, true, coinControl); //presstab s
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, int nSplitBlock, const CCoinControl* coinControl) //presstab
{
    int64 nValue = 0;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true) //presstab qt5
            {
                ...
                int64 nValueIn = 0;
                if (!SelectCoins(nTotalValue, wtxNew.nTime, setCoins, nValueIn, coinControl)) //presstab
                    return false;
                ...
                if (nChange > 0)
                {
                    ...
                    // CPubKey vchPubKey = reservekey.GetReservedKey();  Presstab comment out
                    // assert(mapKeys.count(vchPubKey));

                    // Fill a vout to ourself
                    // TODO: pass in scriptChange instead of reservekey so
                    // change transaction isn't always pay-to-tekcoin-address
                    CScript scriptChange;
                    //scriptChange.SetDestination(vchPubKey.GetID()); presstab comment out
                ...
                int64 nMinFee = wtxNew.GetMinFee(1, false, GMF_SEND, nBytes); //presstab
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, const CCoinControl* coinControl) //presstab
{
    ...
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, 1, coinControl); //presstab
}
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    // Calculate coin age reward
    {
        ...
        nCredit += GetProofOfStakeReward(nCoinAge, nBits, txNew.nTime, false);
    }
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee) //presstab remove txcomment
{
    ...
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired)) //presstab
    {
        string strError;
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64 nValue, CWalletTx& wtxNew, bool fAskFee) //presstab remove txcomment
{
    // Check amount
    if (nValue <= 0)
    ...
    // Parse tekcoin address
    CScript scriptPubKey;
    scriptPubKey.SetDestination(address);

    return SendMoney(scriptPubKey, nValue, wtxNew, fAskFee); //presstab
}
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CtekcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CtekcoinAddress(address).ToString());
}
