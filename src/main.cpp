...
const int64 nChainStartTime = 1376215269; 
...
int64 CTransaction::GetMinFee(unsigned int nBlockSize, bool fAllowFree,
                              enum GetMinFee_mode mode, unsigned int nBytes) const //line changed by presstab
{
    // Base fee is either MIN_TX_FEE or MIN_RELAY_TX_FEE
    int64 nBaseFee = (mode == GMF_RELAY) ? MIN_RELAY_TX_FEE : MIN_TX_FEE;

    //unsigned int nBytes = ::GetSerializeSize(*this, SER_NETWORK, PROTOCOL_VERSION); line deleted by presstab
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            ...
            if (nStakeReward > GetProofOfStakeReward(nCoinAge, pindexBlock->nBits, nTime, false) - GetMinFee() + MIN_TX_FEE)
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // fees are not collected by miners as in bitcoin
...
//        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION)
        if (nTime > 1459976400 && pfrom->nVersion < MIN_PEER_PROTO_VERSION) // Wed, 06 Apr 2016 21:00:00 GMT
        {
