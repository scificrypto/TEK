...
void ThreadNtpSamples(void* parg) {
    const int64 nMaxOffset = 86400; // Not a real limit, just sanity threshold.
    ...
    while (!fShutdown) {
        if (strTrustedUpstream != "localhost") {
            ...
            for (int i = 0; i < nSamplesCount; i++) {
                CNetAddr ip;
                int64 nClockOffset = NtpGetTime(ip) - GetTime();
        ...
        if (GetNodesOffset() == INT_MAX && abs64(nNtpOffset) > 40 * 60)
        {
            ...
            printf("*** %s\n", strMessage.c_str());
            uiInterface.ThreadSafeMessageBox(strMessage+" ", std::string("TEKcoin"), CClientUIInterface::MSG_WARNING);
        }
